...
public class DBUtil {
    private static final String DB_URL = "jdbc:sqlite:bus_schedule.db"; // file in working dir
    ...
    public static void init() {
        try (Connection conn = connect();
            ...
            if (rs.next() && rs.getInt(1) == 0) {
                ...
                ins.setString(1, "student1"); ins.setString(2, "stud123"); ins.setString(3, "student"); ins.executeUpdate();
                ins.close();
            }
            rs.close();
            ps.close();
            ...
            if (rs.next() && rs.getInt(1) == 0) {
                ...
                String[][] data = {
                        {"M3", "7:20 AM", "City", "Campus", "Weekday"},
                        {"M6.4", "7:00 AM", "Campus", "City", "Weekday"},
                        {"M2", "8:30 AM", "Campus", "City", "Weekday"},
                        {"M18", "3:00 PM", "Campus", "City", "Weekday"},
                        {"M5", "4:30 PM", "City", "Campus", "Weekday"}
                };
                for (String[] row : data) {
                    for (int i=0;i<5;i++) insB.setString(i+1, row[i]);
                    insB.executeUpdate();
                }
                insB.close();
            }
